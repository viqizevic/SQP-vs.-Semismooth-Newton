function [x_ssn,it_ssn,t_ssn,x_sqp,it_sqp,t_sqp] = test_betts_func(show)
    ...
    c = [ -a; b ];
    tic;
    %[x_ssn,fval_ssn,it_ssn] = active_set_strategy('betts_func','grad_betts_func','hess_betts_func',lambda,A,c,x0,m0,itmax,tol);
    [x_ssn,fval_ssn,it_ssn] = semismooth_newton('betts_func','grad_betts_func','hess_betts_func',lambda,A,c,x0,itmax,tol);
    t_ssn = toc;
    x1 = sprintf('%.3f ',x_ssn);
