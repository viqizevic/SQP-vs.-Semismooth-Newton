...
Ein Punkt $\xopt \in \F$ heißt globale Lösung des Problems~(\ref{P}) oder
globales Minimum von $f$, wenn
\begin{equation}
  f(\xopt) \leq f(x) \qquad \forall x \in \F
\end{equation}
gilt.
Ein Punkt $\xopt \in \F$ heißt strikte globale Lösung des Problems~(\ref{P}) oder
striktes globales Minimum von $f$, wenn
\begin{equation}
  f(\xopt) < f(x) \qquad \forall x \in \F\backslash\{\xopt\}
\end{equation}
gilt.
Ein Punkt $\xopt \in \F$ heißt lokale Lösung des Problems~(\ref{P}) oder
lokales Minimum von $f$, wenn für eine Umgebung $U(\xopt)$ von $\xopt$
\begin{equation}
  f(\xopt) \leq f(x) \qquad \forall x \in U(\xopt) \cap \F
\end{equation}
gilt.
Ein Punkt $\xopt \in \F$ heißt strikte lokale Lösung des Problems~(\ref{P}) oder
striktes lokales Minimum von $f$, wenn für eine Umgebung $U(\xopt)$ von $\xopt$
\begin{equation}
...
Viele Optimierungsverfahren suchen normalerweise nach einem stationärem
Punkt von~$f$. Aber ein stationärer Punkt muss nicht ein globales oder lokales Minimum sein.
...
% TODO: Newton-Verfahren erklären
...
\begin{definition}
Das allgemeine nichtlineare Optimierungsprobleme ist wie folgt definiert:
\begin{align}
       \min\,& f(x) \\
 \text{mit } & g_i(x) \leq 0 \qquad \text{für } i = 1, \ldots, p \\
             & h_j(x) \leq 0 \qquad \text{für } j = 1, \ldots, m
\end{align}
Man schreibt auch normalerweise nur:
\begin{align}
       \min\,& f(x) \\
 \text{mit } & g(x) \leq 0 \\
             & h(x) \leq 0
...
% TODO als Notation woanders platzieren
...
\begin{equation}
  H_f(x) = f''(x) = \left[ \frac{\partial^2 f}{\partial x_i \partial x_j} (x)
    \right]_{i,j}
\end{equation}
...
\begin{equation}
  d^T f''(x) d \geq \alpha ||d||^2 \qquad d \in \ker A.
\end{equation}
