...
Ansonsten heißt es restringiertes Optimierungsproblem.
...
\begin{definition}
\emph{(Globale und lokale Lösung, vgl. Definition 1.1.4 in
\cite[S.~2~f.]{alt})}\\
...
lokales Minimum, wenn für eine
Umgebung\footnote{Für $\epsilon>0$ und $x \in \R^n$ heißt
$U_\epsilon(x) := \{ u \in \R^n \,|\ \norm{u-x} < \epsilon \}$
die $\epsilon$-Umgebung von $x$.
Eine Menge $U$ heißt eine Umgebung von $x$, wenn es $\epsilon > 0$ gibt,
sodass $U_\epsilon(x) \subset U$.}
$U$ von $\xopt$
\begin{equation}
...
oder striktes lokales Minimum, wenn für eine Umgebung $U$ von
$\xopt$
\begin{equation}
...
Es ist hierbei der Unterschied zwischen der linearen Optimierung und
der nichtlinearen Optimierung gut zu erkennen.
Bei der linearen Optimierung muss die Zielfunktion linear sein
(d.\,h., die Zielfunktion muss in der Form
$f(x) = c^T x = \sum_{i=1}^n c_i x_i$ mit $c \in \R^n$ sein)
...
Eine Höhenlinie ist eine Menge von Punkten,
