...
\end{split}
\end{equation}
Die zulässigen Punkte dieses Problems liegen auf einer Gerade
...
Eine wichtige Grundlage zum Lösen des
Problems~\eqref{prob:opt_prob_mit_lin_gl_nebenbed}
...
Eine $(n \times s)$-Matrix $Z$ heißt Nullraum-Matrix von~$A$, wenn für $d \in
\R^n$ gilt
\begin{equation}
d \in \kr A \quad \Leftrightarrow \quad d = Z z \text{ für ein } z \in \R^s.
\end{equation}
D.\,h. $\im Z = \kr A$.
\end{definition}

Sei $w$ eine Lösung der Gleichung $Ax=b$.
...
Man kann folglich Verfahren für unrestringierte Probleme anwenden.
