Abschließend kommt nun das allgemeine nichtlineare Optimierungsproblem.
Das Optimierungsproblem~\eqref{prob:opt_prob_mit_nichtlin_restr} ist
\begin{align*}
  \min_{x \in \R^n}\ & f(x) \\
...
Um die Optimalitätsbedingung des Problems~\eqref{prob:opt_prob_mit_nichtlin_restr}
zu bekommen, definiert man eine sogenannte \emph{Regularitätsbedingung}
(vgl. Definition 7.2.14 in \cite[S.~261]{alt}),
die hier nicht näher betrachtet wird.
Die folgende Definition ist eine spezielle Bedingung,
aus der die Regularitätsbedingung folgt.
Sie kann aber noch aus anderen Bedingungen hergeleitet werden.

\begin{definition}
\emph{(Mangasarian-Fromowitz-Regularitätsbedingung, vgl. Satz 7.2.24 in
\cite[S.~270]{alt})}
Sei $x \in \F$. Mit
...
wenn die Gradienten
\begin{equation}
 \nabla h_j(x),\ j = 1,\ldots,m, \text{ linear unabhängig}
\end{equation}
...
Der folgende Satz gilt eigentlich mit der allgemeinen Regularitätsbedingung.
Es wird aber hier nur die Mangasarian-Fromowitz-Regularitätsbedingung verwendet.
...
Für die hinreichende Bedingung zweiter Ordnung verweisen wir auf Satz 7.3.1
in \cite[S.~273]{alt}.