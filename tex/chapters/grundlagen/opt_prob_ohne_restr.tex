...
Schrittweitenverfahren von Armijo oder das Schrittweitenverfahren von Wolfe-Powell verwenden.
...
D.\,h., wir lösen in jedem Iterationsschritt eigentlich das Problem
\begin{equation}
  \min_{x \in \R^n}\ f(x^k) + \nabla f(x^k)^T (x-x^k)
                 + \frac{1}{2} (x-x^k)^T f''(x^k) (x-x^k),
\end{equation}
...
Wir verweisen auf den Abschnitt 4.3.2 über dieses Verfahren in
\cite[S.~111~ff.]{alt}.