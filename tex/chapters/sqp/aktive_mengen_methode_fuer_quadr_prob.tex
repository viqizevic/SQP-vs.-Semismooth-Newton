...
und zu lösen.
Wir brauchen daher wieder ein anderes Verfahren,
das wir im nächsten Abschnitt besprechen werden,
um das entstehende Teilproblem zu lösen.
...
können wir also eine Schrittweite $\sigma$ berechnen,
damit $f(x^{k+1} := x^k + \sigma d) < f(x^k)$ gilt.

Sei $t$ diese Schrittweite. Es gilt
\begin{align}
...
Wir erhalten also mit $t = 1$ den größten Abstieg.

Wir müssen noch beachten,
dass $x^{k+1} := x^k + td$
...
\end{equation}
Falls $\langle g_j, d \rangle > 0$ gilt,
müssen wir dann vorsichtig sein.
In diesem Fall gilt
\begin{equation}
\langle g_j, x^{k+1} \rangle
  = \langle g_j, x^k \rangle + t \langle g_j, d \rangle
...
Zusammengefasst müssen wir unsere Schrittweite $\sigma_k$
...
Für $d = 0$ lautet die Gleichung~\eqref{eq:notw_bed_teilprob_in_aktive_mengen_verfahren}
\begin{equation}
...
und wir können somit das Verfahren stoppen.

Falls $\mu \ngeq 0$, dann wählen wir den Index $j \in J_k$ mit
\begin{equation}
...
= \mu_j g_j
\end{equation}
Für die letzte Gleichung haben wir o.\,B.\,d.\,A. angenommen, dass $j = p_k$.
...
\begin{compactenum}[1.]
  ...
  \item Löse das Problem~\eqref{prob:teilprob_in_aktive_mengen_verfahren}
  	$\Rightarrow$ Wir bekommen $d^k, \lambda^k$ und $\mu^k$.
...
Es kann gezeigt werden, dass das Verfahren~\eqref{algo:aktive_mengen_verfahren}
