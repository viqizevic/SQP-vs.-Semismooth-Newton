...
\begin{enumerate}
  ...
  \item Berechne den Vektor $y \in \R^m$ als Lösung der Gleichung
  \begin{equation}
    R^T y = b.
  \end{equation}
  ...
  \item Berechne den Vektor $z \in \R^{n-m}$ als Lösung der Gleichung
  \begin{equation}
    B_{22} z = h_2 - B_{21} y.
  \end{equation}
...
%-------------------------------------------------------------------------%
% Erklärung: In der Matrix H finden wir die Nullraum-Matrix Z
%
...
Ad = AY d_y + AZ d_z = AH^T \left(\begin{array}{c} d_y \\ d_z \end{array}\right)
   ...
   = R^T d_y
\end{equation}
und wegen der Invertierbarkeit von der Matrix $R$
\begin{equation}
...
%-------------------------------------------------------------------------%
% Erklärung: Wie wir eine spezielle Lösung der Gleichung Ax = b finden
%

Sei $\xopt$ die optimale Lösung. Dann gibt es $y^*$ und $z^*$ mit
\begin{equation}
...
\begin{equation}
  Aw = Aw + AZz^* = A (w + Zz^*)
     = A (Yy^* + Zz^*) = A \xopt = b
\end{equation}
Der Vektor $w$ ist somit eine spezielle Lösung der Gleichung $Ax=b$.

%-------------------------------------------------------------------------%
% Erklärung: Umformulierung des Problems als ein unrestringiertes
%

Nun können wir unser Problem~\eqref{prob:quad_opt_prob_mit_lin_gl_nebenbed}
wie in~\eqref{prob:opt_prob_unrestr_mithilfe_nullraum_matrix}
so umformulieren:
\begin{equation}
  \min_{z \in \R^{n-m}} f(w + Zz) =: F(z).
\end{equation}
Die Lösung dieses Problems erfüllt
nach~\ref{eq:grad_zero}
die Optimalitätsbedingung
\begin{equation}
  \nabla F(z^*) = 0.
\end{equation}
Wir sehen jetzt $F(z)$ genauer an.
\begin{align}
...
\end{align}
Für den Gradient von $F$ gilt
\begin{equation}
  \nabla F(z) = Z^T QZz + Z^T Q Y y^* + Z^T q.
\end{equation}

Jetzt betrachten wir den Vektor $h$ und die Matrix $B$
in Schritt~\ref{list:matrix_B_in_nullraum_verfahren} an:
\begin{gather}
\left(\begin{array}{c} h_1 \\ h_2 \end{array}\right)
  = -Hq
...
%-------------------------------------------------------------------------%
% Erklärung: Lösung des Problems
%

Die Bedingung $\nabla F(z^*) = 0$ können wir daher als
\begin{equation}
  B_{22} z^* + B_{21} y^* - h_2 = 0.
\end{equation}
schreiben.
Auf diese Weise bestimmen wir in
Schritt~\ref{list:vektor_z_in_nullraum_verfahren}
den Vektor $z^*$
und folglich in
Schritt~\ref{list:opt_loesung_in_nullraum_verfahren}
die Lösung $\xopt$
nach~\ref{eq:zerlegung_von_xopt_in_nullraum_verfahren}.

%-------------------------------------------------------------------------%
% Erklärung: Berechnung des Multiplikators
%

Zum Abschluss berechnen wir in
Schritt~\ref{list:multiplikator_lambda_in_nullraum_verfahren}
den Multiplikator $\lambda$.
Nach Satz~\ref{satz:notw_bed_fuer_prob_mit_lin_gl_nebenbed}
lautet die Optimalitätsbedingung unseres
Problems~\ref{prob:quad_opt_prob_mit_lin_gl_nebenbed}
\begin{equation}
  Q \xopt + q + A^T \lambda = 0.
\end{equation}
Multiplizieren wir diese Gleichung mit $H$, dann erhalten wir
\begin{equation}
...
\end{equation}
Aus dem oberen Teil dieses Gleichungssystems ergibt sich die Gleichung
\begin{equation}
  0 = Y^T Q (Y y^* + Z z^*) - h_1 + R^T \lambda
    = B_{11} y^* + B_{12} z^* - h_1 + R^T \lambda
\end{equation}
zur Berechnung von $\lambda$.