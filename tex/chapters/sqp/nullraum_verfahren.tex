...
\begin{algorithm}
\emph{(Nullraum-Matrix-Verfahren)}
\begin{enumerate}
  \item Finde unitäre $(n \times n)$-Matrix $H$
  und obere $(m \times m)$-Dreiecksmatrix $R$ mit
  \begin{equation}
    H A^T = \left(\begin{array}{c} R \\ 0 \end{array}\right)
  \end{equation}
  \item Berechne die Vektoren $h_1 \in \R^m$ und $h_2 \in \R^{n-m}$
  \begin{equation}
    \left(\begin{array}{c} h_1 \\ h_2 \end{array}\right) := -Hq
  \end{equation}
  \item Berechne die $(n \times n)$-Matrix $B$
  \begin{equation}
    B := H Q H^T
  ...
  \begin{equation}
    R^T u_1 = b
  \end{equation}
  \item Berechne den Vektor $u_2$ als Lösung der Gleichung
  \begin{equation}
