...
\begin{example}
\emph{(Vgl. Beispiel 2.21 in \cite[S.~33]{ulbrich})}\\
Stückweise differenzierbare Funktionen
sind halbglatt,
beispielsweise
\begin{equation}
  t \in \R \mapsto \abs{t}, \quad
  x \in \R^2 \mapsto \max\{ x_1, x_2 \}
  \quad \text{und} \quad
  x \in \R^2 \mapsto \min\{ x_1, x_2 \}.
\end{equation}
\end{example}

\begin{example}
\emph{(Vgl. Abschnitt 2.5.1 in \cite[S.~31]{ulbrich})}\\
Die Funktion $f \colon \R^n \to \R, x \mapsto \norm{x} = \sqrt{x^T x}$
