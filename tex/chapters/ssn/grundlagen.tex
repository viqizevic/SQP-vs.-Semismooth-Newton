...
Sei $D_f \subset V$ die Menge aller Punkte $v \in V$, wo $f$ eine
(Fr\'echet-)Ableitung\footnote{(Fr\'echet-)Ableitung einer Abbildung
$F\colon \R^n \to \R^m$ ist $F'(x) \in L(\R^n,\R^m)$ mit
...
\[\co{M} := \left\{ \sum_{i=1}^k \lambda_i v_i \left| \: k \in \N,\:
v_i \in M,\: \lambda_i \geq 0 \text{ für } i=1,\ldots,k,\: 
...
\begin{example}
\emph{(Vgl. Abschnitt 2.5.1 in \cite[S.~31f]{ulbrich})}\\
Die Funktion
\begin{equation}
