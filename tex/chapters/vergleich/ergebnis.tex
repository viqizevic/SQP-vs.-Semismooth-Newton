...
vorgestellten Testproblemen sind
die implementierten Programme in der Lage,
die angegebene Lösung zu finden.
Die entsprechenden Programme werden jeweils
als SSN und SQP bezeichnet.

Die folgenden zwei Tabellen zeigen die Ergebnisse des Tests.
Die beiden Programme wurden 100 Mal
auf die Testprobleme angewendet
und der Durschnitt der Laufzeiten ist
als die ermittelte Laufzeit zu verstehen.
Die Bezeichnung $T$ steht für die Laufzeit
des Programms zur Lösung des Problems in Millisekunden
und \#\emph{It} bezeichnet die Anzahl der benötigten Iterationen.
Als Abbruchschranke für die beiden Programme
wird $\varepsilon = 10^{-5}$ gewählt.
...
\begin{tabular*}{0.75\linewidth}{@{\extracolsep{\fill}}crrrr}
  ...
  \bottomrule
\end{tabular*}
\caption{Ergebnisse der Programme für
...
liefert SSN eine bessere Leistung bezüglich der Laufzeit
bei den Testproblemen mit weniger als 10 Variablen.
Beim Testproblem~\ref{test_prob:produktionsplanungsprob}
mit 200~Variablen hat SQP eine bessere Laufzeit.

Bei den meisten Testproblemen benötigt SQP
wenigere Anzahl von Iterationen als SSN.
Theoretisch ist das auch zu erwarten,
weil nach Satz~\ref{satz:konvergenz_seq_quad_prog}
eine quadratische Konvergenz bei SQP im besten Fall vorkommen kann,
bei SSN ist es dagegen nach Satz~\ref{satz:konvergenz_halbglattes_newton}
nur superlineare Konvergenz.
Besonders bei Testproblemen mit quadratischer Zielfunktion
beansprucht SQP nur eine Iteration.

Nur bei zwei Fällen braucht SQP mehr Iterationen,
damit die Abbruchbedingung erfüllt wird.
Betrachtet man aber die Iterationen genauer,
kann man eigentlich das Programm schon
bei wenigeren Iterationen abbrechen,
weil die berechnete Lösung schon erreicht wurde.
...
\end{tikzpicture}
\caption{Laufzeiten der Programme für
Testprobleme~\ref{test_prob:prob_v_quad_func}
